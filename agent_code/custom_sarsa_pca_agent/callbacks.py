...
def setup(self):
    ...
    Loads the model from saved state (assumes that the model has been fitted in train_with_saved_states.py).
    ...
    """
    with open(PCA_TRANSFORMER_PATH, "rb") as file:
        self.pca_transformer = pickle.load(file)
...
def act(self, game_state: dict) -> str:
    ...
    If in training mode, returns a rule based action with 85% probability
    ...
    """
    if self.train:
        self.logger.debug("Choosing rule based action.")
        # will return rule based action with 85% probability, random action else
        return act_rule_based(self, game_state)

    self.logger.debug("Querying model for action.")
    x = self.pca_transformer.transform(state_to_array(game_state).reshape(1, -1))
    ...
    if np.abs(np.sort(response)[-1] - np.sort(response)[-2]) < 0.05:
        ...
        self.logger.debug(f'Model chose action {ACTIONS[index]}')
...
def state_to_array(game_state: dict) -> np.array:
    """
    Converts the game state to into an array of size 314.
    ...
    field = game_state['field']  # np.array(width,height)
    coins = game_state['coins']  # [(x,y)]
    bombs = game_state['bombs']  # [((x,y), t)]
    n, s, b, (x, y) = game_state['self']  # (n,s,b,(x,y))

    coin_array = np.zeros(9 * 2)
    if coins:
        for i, (x, y) in enumerate(coins):
            coin_array[2*i] = x
            coin_array[2*i + 1] = y

    bomb_array = np.zeros(1 * 4)  # only 1 agent
    if bombs:
        for i in range(1):
            (x_b, y_b), t_b = bombs[i]
            bomb_array[2*i] = x_b
            bomb_array[2*i + 1] = y_b
            bomb_array[2*i + 2] = t_b

    self_array = np.array([int(b), x, y])  # b indicates whether a bomb action is possible, x and y are the
    # agent's position

    features = np.concatenate((field.flatten(), coin_array, bomb_array, self_array))
...
def act_rule_based(self, game_state):
    ...
    what it contains.
    """
    # for generating training data: choose a random action with
    # probability 15%
    random_prob = .15
    if self.train and random.random() < random_prob:
        self.logger.debug("Choosing action purely at random.")
        return np.random.choice(ACTIONS, p=[.1, .1, .1, .1, .55, .05])
